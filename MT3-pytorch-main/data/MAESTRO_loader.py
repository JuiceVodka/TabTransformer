...
from data.utils import encode_midi, _snote2events, _make_time_sift_events, encode_midi_tab, _snotetab2events, _make_time_sift_events_tab, SplitNoteTab
import librosa
import numpy as np 
...
#/storage/nikolocal/mag/MT3-pytorch-main/data/datasets/maestro-v3.0.0
#previously: /home/esteban/MAESTRO_datase
class MIDIDataset(IterableDataset):
    def __init__(self, root_dir='/storage/nikolocal/mag/MT3-pytorch-main/data/datasets/maestro-v3.0.0', type='train'): 
...
class MIDIDatasetTab(IterableDataset):
    def __init__(self, root_dir='/storage/nikolocal/mag/MT3-pytorch-main/data/datasets/maestro-v3.0.0', type='train'): 
        # type : train, validation, test
        self.type = type
        self.metadata = json.load(open(os.path.join(root_dir, 'maestro-v3.0.0.json')))
    ...
    def _tokenize(self, midi):
        tokens = encode_midi_tab(midi)

        return tokens

   #changed something for testing 
    def _get_random_length_segment(self, row):
        new_row = {}
        input_length = row['inputs'].shape[0]
        sample_length = random.randint(self.config.mel_length, input_length)
    ...
    def _extract_target_sequence_with_indices(self, row): #here check if can be improved, twice called time sift, velocity set to none in some cases
        """Extract target sequence corresponding to audio token segment."""
        events = []
        target_start_time = row['input_times'][0]
        target_end_time = row['input_times'][-1]
        ...
        for snote_tab in row['targets']:
            if cur_time >= target_start_time and cur_time < target_end_time:
                events += _make_time_sift_events_tab(prev_time=cur_time, post_time=snote_tab.time)
                events += _snotetab2events(snote_tab=snote_tab, prev_vel=cur_vel)
                events += _make_time_sift_events_tab(prev_time=cur_time, post_time=snote_tab.time)

            cur_time = snote_tab.time
            cur_vel = snote_tab.velocity
        row['targets'] = events
    ...
    def _target_to_int(self, row):
        #print(row['targets'])
        row['targets'] = [e.to_int() for e in row['targets']]
        #print(f"Converted targets: {max(row['targets'])}")  # Debugging
        return row
    ...
    def _pad_length(self, row):
        ...
        if targets.shape[0] < self.config.event_length:
            eos = torch.ones(1, dtype=targets.dtype, device=targets.device) * TOKEN_END_TAB
            if self.config.event_length - targets.shape[0] - 1 > 0:
                pad = torch.ones(self.config.event_length - targets.shape[0] - 1, dtype=targets.dtype, device=targets.device) * TOKEN_PAD_TAB
                targets = torch.cat([targets, eos, pad], dim=0)
            else:
                targets = torch.cat([targets, eos], dim=0)
    ...
    def _preprocess(self):
        for idx in range(len(self.midi_paths)):
            ...
            for row in rows:
                ...
                #print(f"Yielding training sample for midi idx: {idx}")
                yield row

    def __len__(self): 
        #tweak this
        return len(self.midi_paths)
        #without thisits stuck at epoch 0
...
if __name__ == "__main__":
    dataset = MIDIDatasetTab(type='train')
